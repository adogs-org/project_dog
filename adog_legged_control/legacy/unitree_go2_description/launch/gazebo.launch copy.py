...
def generate_launch_description():
    ...
    spawner_script = Node(
        ...
        arguments=[
            '-p', '["urdf_file", "task_file", "reference_file", "gait_file"]',
            '--param-file', Command([
                {"urdf_file": "$(arg urdf_file)"}, 
                {"task_file": "$(arg task_file)"}, 
                {"reference_file": "$(arg reference_file)"}, 
                {"gait_file": "$(arg gait_file)"}
            ])
        ],
        output='both')
    ...
    append_environment = AppendEnvironmentVariable(
        'GAZEBO_PLUGIN_PATH',
        os.path.join(os.path.join(get_package_share_directory('realsense_ros2_gazebo'), 'share' 'lib'))
    )
    ...
    launchDescriptionObject.add_action(gz_unpause)
