...
#include "adog_legged_controller/apid_auto_tune_ZNmode.h"
...
void apid_auto_tune_ZNmode_init(
  apid_t * pid, apid_auto_tune_ZNmode_t * tuner, ZNMode mode, float targetValue,
  float low_out_range, float up_out_range, int tuneCycles)
{
  pid->auto_pid = tuner;
...
void set_zn_mode(apid_auto_tune_ZNmode_t * tuner, ZNMode zn) { tuner->znMode = zn; }
...
float tune_pid(apid_auto_tune_ZNmode_t * tuner, float input, unsigned long cycle)
{
  ...
  if (!tuner->output && input < tuner->target_value)
  {
    ...
    // float ku = (4.0 * ((tuner->maxOutput - tuner->minOutput) / 2.0)) / (M_PI * (tuner->max -
    // tuner->min) / 2.0);
...
bool isFinished(apid_auto_tune_ZNmode_t * tuner) { return (tuner->i >= tuner->cycles); }

// return number of tuning cycle
int getCycle(apid_auto_tune_ZNmode_t * tuner) { return tuner->i; }