 ...
 * @FilePath:
 * \undefinedc:\Users\LENOVO\Documents\programs\PID\VS_Project\ConsoleApplication1\ConsoleApplication1\pid_exfunc.c
 * @Description: pid库
 */
#include <stdarg.h>
#include <stdio.h>
...
#if (PID_TYPE == float)
      pid->parameter.bias_for_integral = (float)va_arg(ap, double);
...
#if (PID_TYPE == float)
      pid->parameter.integral_limit = (float)va_arg(ap, double);
#elif (PID_TYPE == double)
...
{
  va_list
    ap;  // 声明一个va_list变量	  va_start(ap, dmode);    // 初始化，第二个参数为最后一个确定的形参
...
#if (PID_TYPE == float)
      pid->parameter.kd_.kd_lpf = (float)va_arg(ap, double);
#elif (PID_TYPE == double)
...
#if (PID_TYPE == float)
      pid->parameter.kd_.kd_pre = (float)va_arg(ap, double);
#elif (PID_TYPE == double)
...
void var(int argc, char ** argv)
{
  ...
  switch (_var->type)
  {
    ...
    case _64:
      ...
      var_logInfo(
        "%s is %d\r\n", _var->name,
        *(int32_t *)_var->body);  //由于使用microlib所以只能输出32位字符输出
