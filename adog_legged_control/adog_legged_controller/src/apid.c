 ...
 * @FilePath:
 * \undefinedc:\Users\LENOVO\Documents\programs\PID\VS_Project\ConsoleApplication1\ConsoleApplication1\pid.c
 * @Description:pid库
 */

#include "adog_legged_controller/apid.h"
#include <stdarg.h>
#include <string.h>
#include "stdio.h"
#ifdef __cplusplus
extern "C"
{
  ...
  void inline APID_Hander(apid_t * pid, PID_TYPE cycle) { pid->handle(pid, cycle); }
  PID_TYPE FOLowPassFilter(PID_TYPE In, PID_TYPE LastOut, PID_TYPE a)
  {
   ...
   * @return {*}
   */
  void APID_Pause(apid_t * pid) { pid->flag.run_status = PAUSE; }
  /**
   * @description: 停止pid,输出将恒等于0
   * @param {apid_t*} pid 实例句柄
   * @return {*}
   */
  void APID_STOP(apid_t * pid) { pid->flag.run_status = STOP; }
  /**
  ...
  {
    // 求微分，再低通滤波
    pid->process.differential_bias = pid->process.bias - 2 * pid->process.last_bias +
                                     pid->process.lastlast_bias;  // 本行进行d的计算
    pid->process.differential_bias = FOLowPassFilter(
      pid->process.differential_bias, pid->process.lpf_differential_bias,
      pid->parameter.kd_.kd_lpf);
    pid->process.lpf_differential_bias = pid->process.differential_bias;
    // pid->process.lpf_differential_bias = pid->parameter.kd_lpf *
    //	(3.1415926f * cycle * (pid->process.differential_bias -
    //pid->process.lpf_differential_bias));//本行进行低通滤波 输入输出为d
    //	//(3.1415926f * cycle * (pid->process.bias - 2*pid->process.last_bias +
    //pid->process.lastlast_bias));//本行进行低通滤波 输入输出为d
  }
  /**
  ...
  void d_handle_Increment_Previous(apid_t * pid)
  {
    ...
    pid->process.lpf_differential_bias = pid->process.differential_bias =
      (pid->process.bias - 2 * pid->process.last_bias + pid->process.lastlast_bias) -
      pid->parameter.kd_.kd_pre * (pid->parameter.target - pid->process.last_target);
  }
  ...
  void i_handle_Position_Saturation(apid_t * pid)
  {
    ...
    //		else if (pid->process.integral_bias * pid->parameter.ki <
    //-pid->parameter.integral_limit) 			pid->process.integral_bias = -pid->parameter.integral_limit *
    //pid->parameter.ki;
  ...
  {
    // 变速积分 -- 可自行添加函数或处理算法
    pid->process.integral_bias +=
      (pid->process.bias + pid->process.last_bias) / (2.0f * pid->cycle);
    pid->process.integral_bias -= (pid->process.bias - pid->process.last_bias) * pid->cycle;
  }
  /**
  ...
  void static __PID_Out(apid_t * pid)
  {
    ////////////////////////////////输出合成
#if USE_HOOK_FIRST
    if (pid->user_hook_first) pid->user_hook_first(pid);
#endif
    // 计算输出
...
#if USE_HOOK_OUT
    if (pid->user_hook_end) pid->user_hook_end(pid);
#endif
    // 输出限幅
...
#define __OPERATE_STATUS()                        \
  ...
  void _PID_Hander(apid_t * pid, PID_TYPE cycle)
  {
    ...
    ////////////////////////////////预测操作--预测值直接加入偏差
    pid->process.predict =
      pid->parameter.k_pre * pid->parameter.predict * ABS(pid->parameter.predict);
    /////////////////////////////////////////////////////////////////////////////////
    ...
    PID_TYPE temp_bias = (pid->parameter.target) - (pid->parameter.present) - pid->process.predict;

    if (pid->parameter.bias_dead_zone > 0)
      temp_bias =
        (temp_bias < pid->parameter.bias_dead_zone && temp_bias > -pid->parameter.bias_dead_zone)
          ? 0
          : temp_bias;  // 误差死区判断--不用该功能时可以将其置-1 0

    if (pid->parameter.bias_limit > 0)
    {
      temp_bias = (temp_bias > pid->parameter.bias_limit)
                    ? pid->parameter.bias_limit
                    : temp_bias;  // 误差限幅--不用该功能时可以将其置-1 0
...
#if APID_USING_FUZZY
    if (pid->fuzzy_ctrl) pid->fuzzy_ctrl(pid);
#endif
#if USE_HOOK_PRE_CPLT
    if (pid->user_hook_pre_cplt) pid->user_hook_pre_cplt(pid);
#endif
    ////////////////////////////////积分操作
    pid->i_handle(pid);
    ////////////////////////////////微分操作
   ...
   * @return {*}
   */
  void inline APID_Set_Out_Limit(apid_t * pid, PID_TYPE value) { pid->parameter.out_limit = value; }
  /**
   * @description: 设置前馈系数
   * @param {apid_t*} pid 实例句柄
   * @param {PID_TYPE} value
   * @return {*}
   */
  void inline APID_Set_Feedforward(apid_t * pid, PID_TYPE value) { pid->parameter.kf = value; }
  /**
   * @description: 设置预测系数
   * @param {apid_t*} pid 实例句柄
   * @param {PID_TYPE} value
   * @return {*}
   */
  void inline APID_Set_KPre(apid_t * pid, PID_TYPE value) { pid->parameter.k_pre = value; }
  /**
   ...
   * @return {*}
   */
  void inline APID_Set_Target(apid_t * pid, PID_TYPE value) { pid->parameter.target = value; }
  /**
   ...
   * @return {*}
   */
  void inline APID_Set_Present(apid_t * pid, PID_TYPE value) { pid->parameter.present = value; }
  /**
   * @description: 设置预测系数
   * @param {apid_t*} pid 实例句柄
   * @param {PID_TYPE} value
   * @return {*}
   */
  void inline APID_Set_Predict(apid_t * pid, PID_TYPE value) { pid->parameter.predict = value; }
   ...
   * @return {PID_TYPE}
   */
  PID_TYPE inline APID_Get_Out(apid_t * pid) { return pid->parameter.out; }
   ...
   * @return {PID_TYPE}
   */
  PID_TYPE APID_Get_Target_Limit(apid_t * pid) { return pid->parameter.target_limit; }
   ...
   * @return {PID_TYPE}
   */
  PID_TYPE APID_Get_Bias_Limit(apid_t * pid) { return pid->parameter.bias_limit; }
   ...
   * @return {PID_TYPE}
   */
  PID_TYPE APID_Get_Bias_Dead_Zone(apid_t * pid) { return pid->parameter.bias_dead_zone; }
   ...
   * @return {PID_TYPE}
   */
  PID_TYPE APID_Get_Integral_Limit(apid_t * pid) { return pid->parameter.integral_limit; }
   ...
   * @return {PID_TYPE}
   */
  PID_TYPE APID_Get_Out_Limit(apid_t * pid) { return pid->parameter.out_limit; }
   ...
   * @return {PID_TYPE}
   */
  PID_TYPE APID_Get_Feedforward(apid_t * pid) { return pid->parameter.kf; }
   ...
   * @return {PID_TYPE}
   */
  PID_TYPE APID_Get_KPre(apid_t * pid) { return pid->parameter.k_pre; }
   ...
   * @return {PID_TYPE}
   */
  PID_TYPE inline APID_Get_Target(apid_t * pid) { return pid->parameter.target; }
   ...
   * @return {PID_TYPE}
   */
  PID_TYPE inline APID_Get_Present(apid_t * pid) { return pid->parameter.present; }
   ...
   * @return {PID_TYPE}
   */
  PID_TYPE inline APID_Get_Predict(apid_t * pid) { return pid->parameter.predict; }
