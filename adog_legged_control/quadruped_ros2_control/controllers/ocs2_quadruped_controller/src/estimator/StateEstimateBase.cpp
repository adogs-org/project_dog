...
namespace ocs2::legged_robot {
    ...
    StateEstimateBase::StateEstimateBase(PinocchioInterface pinocchio_interface, CentroidalModelInfo info,
        ...
        : ctrl_component_(ctrl_component),
          ...
          info_(std::move(info)),//stdmove方法
          ee_kinematics_(ee_kinematics.clone()),//clone 方法
          rbd_state_(vector_t::Zero(2 * info_.generalizedCoordinatesNum)), node_(std::move(node)) {//维度是两倍
        odom_pub_ = node_->create_publisher<nav_msgs::msg::Odometry>("odom", 10);//创造了odom话题，缓存队列为10条
        pose_pub_ = node_->create_publisher<geometry_msgs::msg::PoseWithCovarianceStamped>("pose", 10);//同理
    }

    void StateEstimateBase::updateJointStates() {
        const size_t size = ctrl_component_.joint_effort_state_interface_.size();//读取接口数组的大小
        vector_t joint_pos(size), joint_vel(size);//存储位置和速度
    ...
    void StateEstimateBase::updateImu() {
        ...
        const vector3_t zyx = quatToZyx(quat_) - zyx_offset_;//得到姿态角
        ...
        updateAngular(zyx, angularVelGlobal);//计算全局坐标下的角速度
    }
    ...
    void StateEstimateBase::publishMsgs(const nav_msgs::msg::Odometry &odom) const {
        rclcpp::Time time = odom.header.stamp;//更新社会时间戳
