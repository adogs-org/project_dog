...
namespace ocs2::legged_robot {
    KalmanFilterEstimate::KalmanFilterEstimate(PinocchioInterface pinocchio_interface,
        ...
        : StateEstimateBase(std::move(pinocchio_interface), std::move(info), ee_kinematics, ctrl_component,
                            node),
          numContacts_(info_.numThreeDofContacts + info_.numSixDofContacts),//接触点的数量
          dimContacts_(3 * numContacts_),//接触点的维度
          numState_(6 + dimContacts_),//状态估计的维度
          numObserve_(2 * dimContacts_ + numContacts_) {//观测矩阵的维度
        xHat_.setZero(numState_);//状态估计值 维度跟numstate一样
        ps_.setZero(dimContacts_);//接触点位置
        vs_.setZero(dimContacts_);//接触点速度
        a_.setIdentity(numState_, numState_);//a_ 和 b_：这是卡尔曼滤波器的状态转移矩阵 A 和控制输入矩阵 B。
        b_.setZero(numState_, 3);        //a_ 初始化为单位矩阵，b_ 初始化为零矩阵。
...
//填充从c1 c2矩阵 常用于从状态空间映射到观测空间的转换，c1 和 c2 分别对应于位置和速度等状态的映射。
