...
#include "adog_legged_hardware_interface/serialib.h"
...
char serialib::openDevice(
  const char * Device, const unsigned int Bauds, SerialDataBits Databits, SerialParity Parity,
  SerialStopBits Stopbits)
{
#if defined(_WIN32) || defined(_WIN64)
  // Open serial port
  hSerial = CreateFileA(
    Device, GENERIC_READ | GENERIC_WRITE, 0, 0, OPEN_EXISTING, /*FILE_ATTRIBUTE_NORMAL*/ 0, 0);
...
int serialib::writeString(const char * receivedString)
{
  ...
  size_t Lenght = strlen(receivedString);
  // Write the string
  if (write(fd, receivedString, Lenght) != (ssize_t)Lenght) return -1;
  // Write operation successfull
  return 1;
#endif
}
...
int serialib::readString(
  char * receivedString, char finalChar, unsigned int maxNbBytes, unsigned int timeOut_ms)
{
  ...
  while (nbBytes < maxNbBytes)
  {
    ...
    timeOutParam = timeOut_ms - (unsigned int)timer.elapsedTime_ms();
...
int serialib::readBytes(
  void * buffer, unsigned int maxNbBytes, unsigned int timeOut_ms, unsigned int sleepDuration_us)
{
#if defined(_WIN32) || defined(_WIN64)
  ...
  while (timer.elapsedTime_ms() < timeOut_ms || timeOut_ms == 0)
  {
    ...
    // Try to read a byte on the device
    ssize_t Ret = read(fd, (void *)Ptr, (maxNbBytes - NbByteRead));
    // Error while reading
    if (Ret == -1) return -2;
...
/*!
    ...
    Note that when using serial over USB on Unix systems, a delay of 20ms may be necessary before
   calling the flushReceiver function \return If the function succeeds, the return value is nonzero.
